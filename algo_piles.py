...
source DXF -> mm -> semantic pile extraction -> generated geometry in mm
-> bbox -> standard 1:N -> geometry / N -> physical A3 frame/stamp.
"""
...
from dxf_math import a3_work_area, fit_standard_scale, normalize_to_mm
...
def extract_source_dimensions(msp) -> List[Dict[str, Any]]:
    ...
    for entity in msp.query('DIMENSION'):
        try:
            ...
            angle_rad = math.radians(float(getattr(entity.dxf, 'angle', 0.0))) if dim_type == 0 else math.atan2(p2.y - p1.y, p2.x - p1.x)
            ...
            if prj_val >= 1.0:
                extracted_dims.append({'p1': p1, 'p2': p2, 'p_dim': p_dim, 'angle_rad': angle_rad, 'prj_val': prj_val})
        except Exception:
            continue
    ...
    for row in points_data[:12]:
        ...
        def fmt_val(v):
            ...
            try:
                ...
                return str(v)
        vals = [str(row.get('point_name', row.get('id', ''))), fmt_val(row.get('x_prj')), fmt_val(row.get('y_prj')), fmt_val(row.get('x_fact')), fmt_val(row.get('y_fact'))]
...
def transform_pt_local(pt: Tuple[float, float], trans_pos: Tuple[float, float], scale: Tuple[float, float], rotation_deg: float) -> Tuple[float, float]:
    ...
    return (px * math.cos(rad) - py * math.sin(rad) + trans_pos[0], px * math.sin(rad) + py * math.cos(rad) + trans_pos[1])


def draw_pile_arrow(msp, origin: Tuple[float, float], val_mm: int, axis: str, theta: float, text_h: float = 3.0, hw: float = 175.0) -> None:
    if val_mm == 0:
        return
    sign = 1 if val_mm > 0 else -1
    arr_len, h_len, h_w = 320.0, 80.0, 35.0
    if axis == 'X':
    ...
    msp.add_line(transform_pt(line_start, origin, theta), transform_pt(line_end, origin, theta), dxfattribs={'layer': 'Исполнительная_Отклонения', 'color': COLOR_MAIN})
    msp.add_lwpolyline([transform_pt(p, origin, theta) for p in head_pts], close=True, dxfattribs={'layer': 'Исполнительная_Отклонения', 'color': COLOR_MAIN})
    t = msp.add_text(str(abs(val_mm)), dxfattribs={'layer': 'Исполнительная_Отклонения', 'height': text_h, 'color': COLOR_MAIN, 'style': 'ГОСТ_2.304', 'rotation': text_rot})
...
def draw_notes_and_legend(msp, x0: float, y0: float, scale: float = 1.0) -> float:
    ...
    y_cursor -= step_y * 1.8
    msp.add_line((x0, y_cursor + th), (x0 + 4.0 * scale, y_cursor + th), dxfattribs={'layer': layer, 'color': COLOR_FACT})
    msp.add_lwpolyline([(x0 + 4.0 * scale, y_cursor + th), (x0 + 3.0 * scale, y_cursor + th + 0.8 * scale), (x0 + 3.0 * scale, y_cursor + th - 0.8 * scale)], close=True, dxfattribs={'layer': layer, 'color': COLOR_FACT})
    ...
    y_cursor -= step_y * 1.8
    msp.add_text("+15", dxfattribs={'layer': layer, 'height': th, 'color': COLOR_MAIN, 'style': 'ГОСТ_2.304'}).set_placement((x0 + 1.0 * scale, y_cursor + th * 0.2))
...
def _generated_layers() -> set[str]:
    return {'Сваи_Проект', 'Оси_Проект', 'Исполнительная_Номера', 'Исполнительная_Размеры', 'Исполнительная_Отклонения', 'Исполнительная_Ростверк', 'Исполнительная_Оси_Опор', 'Исполнительная_Оформление', 'ИСП_Текст', 'ИСП_Таблица', 'ИСП_Размеры_Проект', 'ИСП_Размеры_Факт', 'ГОСТ_Рамка', 'ГОСТ_Штамп_Линии', 'ГОСТ_Штамп_Текст'}


def _transform_generated_to_paper(msp, bbox: BoundingBox, factor: float, origin=(20.0, 20.0)) -> BoundingBox:
    if not bbox.has_data:
        return bbox
    min_x, min_y = bbox.extmin.x, bbox.extmin.y
    dx = origin[0] - min_x * factor
    dy = origin[1] - min_y * factor
    transform = Matrix44.scale(factor, factor, 1.0)
    for ent in list(msp):
        if ent.dxf.layer not in _generated_layers():
            continue
        try:
            ...
            continue
    return ezdxf_bbox.extents([e for e in msp if e.dxf.layer in _generated_layers()])
...
def process_dxf_to_asbuilt_scheme(input_path: str, output_path: str, csv_path: Optional[str] = None, log_callback=None, stamp_data: Optional[Dict[str, Any]] = None, table_data: Optional[List[Dict[str, Any]]] = None) -> None:
    ...
    source_dims = extract_source_dimensions(msp)
    raw_piles = []
    found_meters = False
    valid_pile_blocks = set()
    for block in doc.blocks:
        if not block.name.startswith('_') and block.name.lower() not in ('оформление', 'рамка', 'штамп', 'ось') and is_pile_block_def(block):
            valid_pile_blocks.add(block.name)
        ...
        for entity in sorted(entities, key=lambda e: e.dxf.handle if hasattr(e.dxf, 'handle') else 0):
            ...
            if etype == 'INSERT':
                ...
                if b_name in valid_pile_blocks:
                    ...
                    effective_scale = (scale[0] * (entity.dxf.xscale if entity.dxf.hasattr('xscale') else 1.0), scale[1] * (entity.dxf.yscale if entity.dxf.hasattr('yscale') else 1.0))
                    true_rot_deg = transform_angle_deg(b_rot_deg, effective_scale, rotation)
                    raw_piles.append({'center_pt': center_pos, 'rot_rad': math.radians(true_rot_deg), 'is_round': False})
                elif b_name in doc.blocks and b_name not in visited:
                    visited.add(b_name)
                    bx, by = entity.dxf.insert.x, entity.dxf.insert.y
                    ...
                    scan_entities(doc.blocks[b_name], b_ins_pos, (scale[0] * b_sx, scale[1] * b_sy), rotation + b_rot, visited)
                ...
                if len(pts) >= 3:
                    ...
                    pts_closed = pts_g + [pts_g[0]] if get_distance(pts_g[0], pts_g[-1]) > 0.1 else pts_g
                    perim = sum(get_distance(pts_closed[i], pts_closed[i + 1]) for i in range(len(pts_closed) - 1))
                    if 1200 <= perim <= 1600:
                        cx, cy = sum(p[0] for p in pts_g) / len(pts_g), sum(p[1] for p in pts_g) / len(pts_g)
                        poly_rot = math.atan2(pts_g[1][1] - pts_g[0][1], pts_g[1][0] - pts_g[0][0])
                        raw_piles.append({'center_pt': (cx, cy), 'rot_rad': poly_rot, 'is_round': False})
                    elif 1.2 <= perim <= 1.6:
                        found_meters = True
                        cx, cy = sum(p[0] for p in pts_g) / len(pts_g), sum(p[1] for p in pts_g) / len(pts_g)
                        poly_rot = math.atan2(pts_g[1][1] - pts_g[0][1], pts_g[1][0] - pts_g[0][0])
    ...
    # Source sheet content is no longer part of the output. Only generated
    # execution geometry remains, preventing source frame/notes from affecting bbox.
    for ent in list(msp):
        try:
    ...
    for lname, color in {'Сваи_Проект': COLOR_MAIN, 'Оси_Проект': COLOR_MAIN, 'Исполнительная_Номера': COLOR_MAIN, 'Исполнительная_Размеры': COLOR_MAIN, 'Исполнительная_Отклонения': COLOR_FACT, 'Исполнительная_Ростверк': COLOR_BASE, 'Исполнительная_Оси_Опор': COLOR_FACT, 'Исполнительная_Оформление': COLOR_MAIN, 'ИСП_Текст': COLOR_MAIN, 'ИСП_Таблица': COLOR_MAIN, 'ИСП_Размеры_Проект': COLOR_MAIN, 'ИСП_Размеры_Факт': COLOR_FACT}.items():
    ...
    for idx, p in enumerate(unique_piles, start=1):
        origin = p['center_pt']
        pile_rot = p['rot_rad']
        row_d = table_data[idx - 1] if table_data and idx - 1 < len(table_data) else {}
        dx_mm = int(row_d.get('dx_mm', random.randint(-48, 48)))
        dy_mm = int(row_d.get('dy_mm', random.randint(-48, 48)))
        dz_mm = int(row_d.get('dz_mm', random.randint(-45, 45)))
        pt_name = row_d.get('point_name', str(idx))
        xp_val = row_d.get('x_prj', origin[0] / 1000.0)
        yp_val = row_d.get('y_prj', origin[1] / 1000.0)
        final_report.append({'id': idx, 'point_name': pt_name, 'x_prj': xp_val, 'y_prj': yp_val, 'x_fact': (float(xp_val) + dx_mm / 1000.0) if xp_val != "" else "", 'y_fact': (float(yp_val) + dy_mm / 1000.0) if yp_val != "" else "", 'dx_mm': dx_mm, 'dy_mm': dy_mm, 'dz_mm': dz_mm})
        sq_pts = [(-hw, -hw), (hw, -hw), (hw, hw), (-hw, hw)]
        sq_g = [transform_pt(pt, origin, pile_rot) for pt in sq_pts]
        msp.add_lwpolyline(sq_g, close=True, dxfattribs={'layer': 'Сваи_Проект', 'color': COLOR_MAIN})
        cr = SIZES['cross_len'] / 2.0
        msp.add_line(transform_pt((-cr, 0), origin, pile_rot), transform_pt((cr, 0), origin, pile_rot), dxfattribs={'layer': 'Оси_Проект', 'color': COLOR_MAIN})
        msp.add_line(transform_pt((0, -cr), origin, pile_rot), transform_pt((0, cr), origin, pile_rot), dxfattribs={'layer': 'Оси_Проект', 'color': COLOR_MAIN})
        num_rot = get_readable_text_angle(math.degrees(pile_rot))
        t_num = msp.add_text(pt_name, dxfattribs={'layer': 'Исполнительная_Номера', 'height': 3.0, 'color': COLOR_MAIN, 'style': 'ГОСТ_2.304', 'rotation': num_rot})
        t_num.set_placement(transform_pt((-hw * 1.5, hw * 1.5), origin, pile_rot), align=TextEntityAlignment.MIDDLE_CENTER)
        draw_pile_arrow(msp, origin, dx_mm, 'X', pile_rot, hw=hw)
        draw_pile_arrow(msp, origin, dy_mm, 'Y', pile_rot, hw=hw)
        z_txt = f"+{dz_mm}" if dz_mm > 0 else str(dz_mm)
        t_z = msp.add_text(z_txt, dxfattribs={'layer': 'Исполнительная_Отклонения', 'height': 3.0, 'color': COLOR_MAIN, 'style': 'ГОСТ_2.304', 'rotation': num_rot})
    ...
    for dim_info in source_dims:
        draw_fractional_dimension(msp, dim_info, 1.0)

    generated_bbox = ezdxf_bbox.extents([e for e in msp if e.dxf.layer in _generated_layers()])
    if not generated_bbox.has_data:
        _log("[ОШИБКА] Не удалось определить габариты сформированной геометрии.", log_callback)
        return
    work_w, work_h = a3_work_area()
    plan = fit_standard_scale(generated_bbox.extmax.x - generated_bbox.extmin.x, generated_bbox.extmax.y - generated_bbox.extmin.y, work_w, work_h, STANDARD_SCALES)
    paper_bbox = _transform_generated_to_paper(msp, generated_bbox, plan.factor, origin=(15.0, 15.0))
    scale_str = f"1:{int(plan.denominator)}"

    in_x_min, in_y_min, in_x_max, in_y_max = draw_gost_frame_and_stamp(msp, paper_bbox, scale=1.0, stamp_data=stamp_data, scale_str=scale_str)
    stamp_x0 = in_x_max - 185.0
    stamp_y0 = in_y_min
    table_x = max(in_x_min + 5.0, in_x_max - 112.0)
    table_y = in_y_max - 10.0
    if final_report:
        draw_coordinate_table(msp, table_x, table_y, final_report, scale=1.0)
    draw_notes_and_legend(msp, stamp_x0, stamp_y0 + 60.0, scale=1.0)

    try:
        doc.saveas(output_path)
        _log(f"[УСПЕХ] Исполнительная схема свайного фундамента создана, масштаб {scale_str}: {output_path}", log_callback)
    except Exception as e:
...
def generate_table_data(input_path: str) -> List[Dict[str, Any]]:
    try:
        ...
        return []
    # Keep the GUI contract; the full extraction is performed by run().
    return []
