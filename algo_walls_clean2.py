...
def _filtered_source_msp(doc):
    """Return the source modelspace unchanged.

    Disposable/presentation-only geometry (hatch, wipeout, ACI-green helper
    entities) is rejected inside ``extract_valid_geometry`` itself via
    ``is_disposable_geometry()`` in algo_walls.py. A duplicate, unused color/
    type filter used to live here (dead code — never called, which is why
    green pile geometry and source hatching were leaking into generated
    "Откосные стенки" sheets); it has been removed in favor of the single
    implementation in algo_walls.py so there is only one place to maintain.
    """
