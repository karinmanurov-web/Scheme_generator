...
Loaded after the legacy walls plugin so the UI uses this implementation while
keeping the legacy ``algo_walls`` module available as the geometry helper.
"""
...
def run(input_dxf: str, output_dxf: str, output_csv: Optional[str] = None,
    ...
    # Never reuse the source DXF as the presentation template. Build a clean
    # document so source frames, stamps, hatches and helper entities cannot leak.
    ...
    for item in elements:
        if item[0] == "LINE":
            shifted_elements.append(("LINE", (item[1][0] + dx, item[1][1] + dy),
                                     (item[2][0] + dx, item[2][1] + dy), item[3]))
        elif item[0] == "POLYLINE":
            shifted_elements.append(("POLYLINE", [(x + dx, y + dy) for x, y in item[1]], item[2], item[3]))
        elif item[0] == "CIRCLE":
            shifted_elements.append(("CIRCLE", (item[1][0] + dx, item[1][1] + dy), item[2], item[3]))
    ...
    in_x_min, in_y_min, in_x_max, in_y_max = draw_gost_frame_and_stamp(
        msp, drawing_box, scale=text_scale, stamp_data=stamp_data, scale_str=scale_str)

    # Keep tables and notes inside the generated frame and relative to the
    # actual stamp position, rather than relative to source-DXF coordinates.
    ...
    draw_legend_and_notes(msp, (chrome_left, notes_top), scale=text_scale,
