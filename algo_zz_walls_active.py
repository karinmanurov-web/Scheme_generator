...
from algo_walls import STANDARD_SCALES, analyze_wall_geometry, draw_legend_and_notes, draw_quantities_table, setup_document
...
def _fit_scale(w,h):
    req=max(w/(SHEET_W-INNER_LEFT-INNER_RIGHT),h/(SHEET_H-INNER_BOTTOM-INNER_TOP),1.0)
    return next((float(s) for s in STANDARD_SCALES if float(s)>=req),float(STANDARD_SCALES[-1]))

def _draw_elements(msp,elements):
    for x in elements:
        if x[0]=="LINE": msp.add_line(x[1],x[2],dxfattribs={"layer":x[3],"color":1 if x[3]=="ГОСТ_Оси" else 7})
        elif x[0]=="POLYLINE": msp.add_lwpolyline(x[1],close=x[2],dxfattribs={"layer":x[3],"color":7})
...
def _scale_element(x,f,dx,dy):
    ...
    if x[0]=="POLYLINE": return ("POLYLINE",[((a+dx)*f,(b+dy)*f) for a,b in x[1]],x[2],x[3])
...
def run(input_dxf:str,output_dxf:str,output_csv:Optional[str]=None,log_callback=None,stamp_data:Optional[Dict[str,Any]]=None,table_data:Optional[List[Dict[str,Any]]]=None)->None:
    src=ezdxf.readfile(input_dxf); elements,dims,levels=extract_geometry(src.modelspace(),src); geom=_geometry_bbox(elements)
    if geom is None: raise RuntimeError("Не удалось извлечь исполнительную геометрию")
    min_x,min_y,max_x,max_y=geom; w,h=max_x-min_x,max_y-min_y; denominator=_fit_scale(w,h); f=1.0/denominator; pad=min(max(w,h)*.02,20.0); dx,dy=-min_x+pad,-min_y+pad
    out=setup_document(ezdxf.new("R2018",setup=True)); msp=out.modelspace(); _draw_elements(msp,[_scale_element(x,f,dx,dy) for x in elements])
    dims=[_scale_dim(x,f) for x in dims]
    for d in dims: _draw_dimension(msp,d)
    for x in levels:
        d=dict(x); d["pt"]=((x["pt"][0]+dx)*f,(x["pt"][1]+dy)*f); _draw_level(msp,d)
    box=ezdxf_bbox.extents(msp); scale_str=f"1:{int(denominator)}"; ix0,iy0,ix1,iy1=draw_gost_frame_and_stamp(msp,box,scale=1.0,stamp_data=stamp_data,scale_str=scale_str)
